...
unsigned int search_L2_TLB (L2_TLB* l2_tlb, unsigned int page_number) {
    ...
    // Entry found in L2 TLB -- return frame number
    if (i < NUM_L2_TLB_WAYS) 
        return frame_number;
        
    // Entry NOT found in L2 TLB -- set search flag to L2_TLB_MISS
    else 
        return 6553699; // Returning an invalid frame number to indicate a miss -- anything outside 0 <= frame_number < 65536( = 2^(25-9)) but within unsigned int range works // return -73;
}
...
void update_L1_TLB (L1_TLB* l1_tlb, unsigned int page_number, unsigned int frame_number, unsigned int shared_bit) {
    ...
    if (i == NUM_L1_TLB_WAYS) {
        ...
        // need to update L2 TLB with this entry.
    }
}
